...
- **EPSS exploit-probability scores:** every cve-bin-tool finding carries `epss_probability` / `epss_percentile` (≈354k scores in the local DB, refreshed offline — the EPSS CDN is proxy-throttled in-contour; procedure in [`docs/big-artifacts.md`](docs/big-artifacts.md)).
- **SBOM-first deliveries:** when the delivery *is* an SBOM (vendor ships CycloneDX/SPDX instead of the binaries), those documents are detected in the extracted tree **by content** and merged with Syft's own SBOM into `artifacts/sbom/scan-input.cdx.json`, which is what the matchers read. Merge, not replace — a delivery may hold both an SBOM and real software. Wired as a `depends_on` of `grype-scanner`, so every entry point (dashboard, `run-scan.sh`, `scan_archive.sh`, hand-typed compose runs) gets it for free.
