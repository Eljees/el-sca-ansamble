...
- **Движки обновлены:** Syft `v1.51.1`, Grype `v0.118.0`, Trivy `0.74.0`.
...
- **Гигабайтные артефакты — штатный маршрут.** Веб-загрузка для файлов от
  ~500 МБ не рекомендуется; вместо неё: доставка на сервер (rsync с докачкой /
  WinSCP / scp) → регистрация в каталоге **без** HTTP-загрузки
  ([`scripts/register_local_artifact.sh`](scripts/register_local_artifact.sh):
  hardlink → 0 лишних байт, полноценная карточка) → скан с карточки или флагом
  `-s`. Пошаговый гайд, включая настройку WinSCP по шагам:
  [`docs/big-artifacts.md`](docs/big-artifacts.md). Проверено на 3.4 ГБ:
  доставка 5,5 мин, скан ~4 мин, на диске файл лежит один раз.
- **Входы ≥ 512 МиБ больше не копируются в run-папку** — hardlink вместо копии
  (`ccc9a87`), место не удваивается на каждый прогон.

## Что нового (2026-07-16)

- **Отчёт называет объект и его хэши.** Веб-обзор и `.md` больше не показывают
  заглушку `Target: /absolute/path/...` и `UNKNOWN`: в шапке — имя загруженного
  архива (напр. `SCAgent_Linux_12_5_1.zip`) и `CYBERSEC-id`, а в разделе хэшей —
  **MD5 + SHA-1 + SHA-256** и для входного архива, и для финальной цели
  (распакованного дерева). Дайджесты считаются один раз в extractor'е и кладутся
  в `summary.json`; кнопка Reports открывает **самый свежий** ран артефакта.
  (`c2a92c6`)
...
- **Удаление артефакта из хранилища.** Кнопка «🗑 Удалить навсегда» с тремя
  ...
  это evidence.
- **Тема Rose Phosphor.** Фосфорно-зелёный текст, розовый хром, моношрифт.
  Фон-эффекты живут в слоях `background` у `body`, а не оверлеем поверх текста.
  Бочки с мутагеном остались кислотно-зелёными: их цвет несёт смысл.
- **Десять палитр сохранены** в [`resilient_updates/themes.py`](resilient_updates/themes.py)
  и показываются в шапке под кнопкой `🎨 Темы`. Выбор пока **отключён** —
  переключение тем в планах (см. Roadmap в [`PROJECT_OVERVIEW.md`](PROJECT_OVERVIEW.md)).
...
- `cve-bin-tool` scan может занимать долгое время — и на крупных Go-бинарях (>50 МБ), и, с тех пор как вложенные архивы распаковываются полностью, на больших SBOM (лукап идёт по каждому компоненту: 25 тыс. компонентов = десятки минут). По умолчанию таймаут 3600 с, настраивается через `CVE_BIN_TOOL_SCAN_TIMEOUT_SECONDS`. При таймауте пишется пустой отчёт и пайплайн продолжается — то есть слишком малый таймаут **молча стоит вам всех находок cve-bin-tool**; в отчёте это видно как `tool_failures: ['cve-bin-tool']`.
...
- `cve-bin-tool` scan не стартует или завис:
  ...
  - если scan висит долго — это нормально для крупных Go-бинарей (100+ МБ) и для больших SBOM; таймаут задаётся `CVE_BIN_TOOL_SCAN_TIMEOUT_SECONDS` (дефолт 3600), при достижении пишется пустой отчёт и пайплайн продолжается
