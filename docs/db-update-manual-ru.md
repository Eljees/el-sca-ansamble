...
## 0. Актуальное состояние (2026-09-21)

Последнее обновление — 21.09.2026. Возраст всех трёх баз на момент проверки
меньше двух часов, контрольный прогон регрессии не показал.

| Что | Значение |
|-----|----------|
| Grype DB | свежая, движок `v0.119.0` |
| Trivy DB | свежая, движок `0.74.0` |
| cve-bin-tool | свежая, `3.4`; фиды NVD от 21.09 |
| CVE в базе | 461 204 (из них 72 845 за 2026 год) |
| Диапазонов версий | 3 026 701 |
| EPSS | 377 238 записей, срез 20.09.2026 |
| Разобрано из фидов | 403 189 записей |

Все 26 годовых фидов NVD скачаны и проверены на обрезку (`totalResults`
против фактического числа записей) — битых нет.

### Что изменилось с июля

- **EPSS переехал.** `epss.cyentia.com` отдаёт 301 на
  `epss.empiricalsecurity.com`. Без `curl -L` в файл пишется пустота, и это
  выглядит как битый архив. По новому адресу прокси ноды душит закачку до
  ~130 Б/с, так что 2.6 МБ не вытягиваются — качать по-прежнему с [ХОСТ]
  и слать `scp` (§6).
- **Пересев EPSS через `cve-bin-tool -u latest` зависает.** После строк
  «Disabling data source …» контейнер встаёт намертво: процессор в нуле, лог
  не растёт, EPSS в базе остаётся 0. Не лечится ни отключением прокси, ни
  ожиданием. Рабочий путь — залить csv прямо в SQLite:
  таблица `cve_metrics(cve_number, metric_id, metric_score, metric_field)`,
  `metrics` = {1: EPSS, 2: CVSS-2, 3: CVSS-3}, percentile кладётся в
  `metric_field`; перед вставкой `DELETE FROM cve_metrics WHERE metric_id=1`.
- **`/tmp` на ноде — отдельный раздел 3.8 ГБ.** Стейдж фидов туда не влезает
  вместе с остатками прошлых циклов: curl падает с
  `(23) Failure writing output to destination`, и ошибка выглядит сетевой, а
  не дисковой. Качать в `artifacts/nvd-stage/` на основном диске.
- **Перед переимпортом снимать копию текущего EPSS** из тома
  (`/c/cve-bin-tool/epss/epss_scores-current.csv`): сборка кандидата его
  стирает, и без запасного файла откатиться некуда.
- **Нода может перестать отвечать под переимпортом.** 21.09 на полчаса: порт
  22 принимает TCP, но рвёт соединение до баннера, дашборд молчит. Поднялась
  сама, без перезагрузки; OOM в `journalctl -k` нет, импорт при этом дошёл до
  `feed_rc=0`. Не перезапускать вслепую — подождать и проверить лог импорта.

## 0a. Состояние на 2026-07-17 (историческое)
