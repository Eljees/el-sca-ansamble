...
## 7. Автовыбор маршрута: route-doctor (ADR-0007)

Ручная настройка профилей Compose и `HTTP_PROXY` подходит, когда сеть стабильна. В меняющихся условиях (корп-прокси + домашний VPN + v2rayN на хосте в разных профилях) удобнее **автовыбор**, реализованный в сервисе `route-doctor`.

**Как он вписывается в топологию выше:**

```
         ┌─ scanner-net ──────────────────────────────────────────────────┐
         │                                                                 │
tools ──►│  route-doctor  ──► зондирует: tinyproxy:8888 / proxy-xray:1080 │
         │      │              + host.docker.internal:<порты хоста>        │
         │      │              + direct                                    │
         │      ▼                                                          │
         │  artifacts/route-plan.env  ◄── апдейтеры читают per-tool egress│
         │                                                                 │
         └─────────────────────────────────────────────────────────────────┘
```

Ключевое отличие: зондирование идёт **изнутри** `scanner-net`, поэтому обнаруживает ровно то, что видят сами апдейтеры (а не то, что вы видите с хоста).

**Жёсткое ограничение cve-bin-tool:** инструмент не поддерживает SOCKS-прокси — его Python-клиент понимает только `HTTP_PROXY`. Route-doctor всегда выдаёт ему HTTP-мост (`tinyproxy:8888` или HTTP-прокси хоста), тогда как Trivy/Grype могут идти по `ALL_PROXY=socks5h://…`.

**Связь с разделом 2 (когда включать):**

- При поднятом профиле `proxy` (`tinyproxy` + `proxy-xray`) route-doctor видит их первыми и предпочитает им — поведение идентично ручной строке `HTTP_PROXY=http://tinyproxy:8888`.
- Без профиля `proxy` route-doctor ищет живой прокси прямо на хосте и при его отсутствии делает `direct` — не нужно ни трогать `.env`, ни поднимать sidecar'ы.
- `--no-auto-route` / `EL_SCA_AUTO_ROUTE=0` отключают автовыбор, и в силу вступает ваш `.env` без изменений (обратная совместимость).

Полный справочник — `docs/proxy.md`, раздел «Автовыбор маршрута: route-doctor». ADR с обоснованием — `docs/adr/0007-updates-from-anywhere.md`.
