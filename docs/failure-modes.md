...
| cve-bin-tool scan timeout | cve-bin-tool | scan exceeds `CVE_BIN_TOOL_SCAN_TIMEOUT_SECONDS` (default 600s); `timeout` exits 124 | empty `[]` report written; pipeline continues | silently missed CVEs from binary scan | `[cve-bin-tool] WARN: scan timed out` in container log |
