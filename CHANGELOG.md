...
- **Windows-установщики (exe/msi) получали в отчёт результаты предыдущего
  прогона.** Ветка `FORMAT=win` не запускает экстрактор и Trivy и ничего не
  чистила: в отчёт попадали Trivy-находки, «Extracted archives» и оба SHA-256
  цели, просканированной перед ней (CYBERSEC-14915, 24.09: ActiveGate `.exe`
  и Remote Plugin Module `.msi` показали Jetty и хеши docker-образа).
  Теперь `run-scan.sh` в начале каждого прогона (кроме `--resume`) удаляет
  отчёты сканеров прошлого прогона, `extracted/current` — если этот прогон
  не распаковывает, и рабочие каталоги win/apk-анализаторов.
- **Вложенные установщики не вскрывались.** `analyze_win_installer.py`
  открывал только внешний слой: `OneAgent-Windows.exe` → MSI на 800 МБ
  уходил в сканеры одним файлом (3 компонента, 0 находок против 241
  компонента после ручной распаковки). Добавлен `unpack_nested`: до трёх
  уровней MSI/CAB/ZIP (jar не трогаются), с защитой от zip-slip. Анализатор
  запускается из смонтированного workspace — правка раскатывается `git pull`.
- **Библиотеки внутри Windows-установщиков не проверялись.** PE-анализатор
  читает только VS_VERSIONINFO, jar-ы (Jetty, BouncyCastle, jackson) не
  доходили ни до одного матчера, а Grype видел только PE-SBOM анализатора.
  В win-ветке распакованное дерево теперь каталогизирует Syft (Grype по его
  SBOM находит встроенную JRE и jar-ы) и проверяет Trivy rootfs.
- **Docker-образы проверялись без учёта дистрибутива.** Образ (`docker save`)
  распаковывался в файлы как обычный архив: Grype видел `distro: none` и
  пропускал все пакеты ОС, cve-bin-tool сравнивал голые номера версий
  RHEL-библиотек, не зная о бэкпортах. Образ oneagent (RHEL 9.6): 28 ложных
  Critical и ни одной находки по пакетам ОС; в режиме образа — 0 Critical и
  48 реальных High с версиями исправлений Red Hat. `run-scan.sh` распознаёт
  docker-archive и после файловых сканеров запускает Trivy `image --input` и
  Grype `docker-archive:`; при их сбое остаются файловые отчёты.
- **Отчёт отбрасывает два шаблона ложных срабатываний cve-bin-tool**
  (`collision_filter.filter_compiler_markers`,
  `filter_distro_owned_binaries`): «gcc x.y» — строка компилятора в секции
  `.comment`, а не поставляемый gcc (301 строка на CYBERSEC-14915); файлы
  пакетов ОС в образе с распознанным дистрибутивом — их уже оценила база
  дистрибутива (272 строки). Файлы под `/opt` (библиотеки вендора) остаются.
  Отброшенное перечислено отдельным разделом отчёта. xlsx-отчёт теперь
  применяет ту же цепочку фильтров, что и Markdown (раньше в нём не было
  фильтра платформ).

### Documentation

- **Записано состояние баз после обновления 21.09.2026.**
  `docs/db-update-manual-ru.md` §0 был от 17.07 и устарел: там же описаны
  четыре грабли, всплывшие за это время — переезд EPSS на
  `epss.empiricalsecurity.com` (без `curl -L` получаем пустой файл),
  зависание пересева через `cve-bin-tool -u latest` и обход через прямую
  запись в `cve_metrics`, отдельный раздел `/tmp` на 3.8 ГБ, из-за которого
  закачка фидов падает с виду сетевой ошибкой, и потеря связи с нодой под
  переимпортом (поднимается сама, импорт при этом доходит до конца).
  Прежний срез сохранён как §0a. В `PROJECT_OVERVIEW.md` добавлена сводка по
  базам и поправлена версия проекта — там оставалась `0.1.5`.
...
- **Отчёт заказчику нарушал стилевое правило в собственной формулировке.**
  В разделе «Interpretation» стояло «Находки являются сигналом сканеров, **а не**
  подтвержденной применимостью уязвимостей», и та же конструкция — в подписи
  листа XLSX. Конструкция «не…, а…» в отдаваемых наружу текстах запрещена, так
  что её нёс каждый отчёт, который выпускал конвейер.

  Формулировки переписаны с сохранением смысла: «Находки — это сигнал сканеров;
  подтвержденную применимость уязвимостей они не означают».

  Добавлен гейт `test_customer_facing_prose_avoids_the_banned_construction`,
  который ищет конструкцию **только в строковых литералах** `reporting.py` и
  `report_xlsx.py`. Простой `grep` тут не годится: та же формулировка встречается
  в комментариях `dashboard.py` (CSS и JS), и валить сборку из-за комментария
  незачем — наружу он не попадает. Рядом стоит второй тест, который проверяет,
  что сам гейт срабатывает на заведомо нарушающей строке и молчит на безобидной
  вроде «Второго бага не заявлял».

- **Каталог проекта у MCP-сервера теперь проверяется, а сообщение об ошибке
  называет причину** (CYBERSEC-14231, продолжение #7). `#7` научил сервер
  вычислять корень из своего пути, но `EL_SCA_DIR` по-прежнему выигрывал всегда,
  даже когда указывал туда, где работать невозможно.

  Ровно это и происходило: клиент передавал `EL_SCA_DIR=/mnt/d/dev/el-sca-ansamble`
  — путь, переживший переезд D: → W: с одними `artifacts` и `configs`. Поймано
  прямо на живом процессе: `argv` указывает на **исправленный** файл в
  `/mnt/w/...`, а в окружении лежит старое значение. Всё, что кэширует окружение
  запуска, продолжает подставлять его и после исправления конфигурации, поэтому
  сервер должен защищаться сам.

  Теперь `EL_SCA_DIR` побеждает, когда действительно указывает на проект compose;
  когда указывает на каталог без файла compose, а сам репозиторий проектом
  является, сервер берёт репозиторий и печатает об этом в stderr. Проверяются
  четыре имени: `docker-compose.yml`, `docker-compose.yaml`, `compose.yml`,
  `compose.yaml`.

  Сообщения гейта разделены по причинам. Было одно на все случаи —
  `EL_SCA_DIR not found: <путь>`, и оно говорило «не найдено» про каталог,
  который прекрасно существует. Стало: «каталог не существует» либо «в каталоге
  нет файла compose» — с путём, перечнем искомых имён и упоминанием переменной.

  Заодно две фикстуры приведены к тому, чем они себя называли. Модульная
  `server` и тест на переопределение указывали на ПУСТЫЕ временные каталоги —
  такими `docker compose` воспользоваться не мог никогда. Пустая фикстура
  проходила лишь потому, что переопределение никто не проверял, и после этой
  правки она молча уводила весь модуль на настоящий репозиторий.

  Тесты: подменённое переопределение уступает репозиторию, годное
  переопределение по-прежнему выигрывает, распознавание всех четырёх имён
  compose, и оба сообщения гейта по отдельности.
