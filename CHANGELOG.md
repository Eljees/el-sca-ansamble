...
- **Отсев по платформе из CPE** (CYBERSEC-13702). Правило по Maven `groupId`
  бессильно там, где имя пакета и есть вся личность компонента: NuGet
  `OpenTelemetry` и Go `opentelemetry` — одна и та же строка, опровергать
  нечем. Но NVD хранит платформу в 11-м поле CPE (`target_sw`):
  `cpe:2.3:a:opentelemetry:opentelemetry:*:*:*:*:*:go:*:*`. Экосистема
  компонента известна из purl в SBOM (`pkg:nuget/OpenTelemetry@1.15.3`), и
  если платформа названа и не совпадает — привязка опровергнута тем же
  источником, из которого взята сама находка.

  `cve_platforms_from_feeds()` читает `target_sw` из локальных годовых фидов
  NVD, открывая только те годы, к которым относятся CVE отчёта (плюс
  `modified`), — пара файлов вместо всех 26. Нет фидов, нет `target_sw` или
  стоит `*` — ничего не отсеивается: правило работает только на явном
  доказательстве. Компоненты без purl и экосистемы вне таблицы соответствий
  не трогаются.

  На PIX Process Studio 2.3.3 (.NET, 620 компонентов, все `pkg:nuget/…`)
  снято 8 находок из 9: OpenTelemetry для Go, C++ и Node.js, плагин Jenkins
  и расширение PostgreSQL pgvector. Осталась одна — CVE-1999-1278 на NLog,
  где в CPE стоит `*`; правило её честно не трогает.
  **High/Critical в отчёте не осталось.**

  `summarize_dropped()` теперь копит все основания отсева для одной пары
  (продукт, версия): у OpenTelemetry 1.15.3 часть CVE помечена `go`, часть
  `c++` и `node.js` — схлопывать их в одну строку было бы нечестно.
  Тесты: `tests/test_platform_filter.py` (16).

### Fixed

- **MCP-сервер искал репозиторий по прибитому пути `/mnt/d`** (CYBERSEC-14231).
  `PROJECT_DIR` брался как
  `Path(os.environ.get("EL_SCA_DIR", "/mnt/d/dev/el-sca-ansamble"))`. После
  переезда D: → W: этот каталог остался на диске, но внутри лежали только
  `artifacts` и `configs`, поэтому проверка существования проходила, а каждый
  вызов compose падал с `no configuration file provided: not found` — ошибка,
  которая не называет ни каталог, ни переменную.

  `server.py` живёт по пути `<репозиторий>/tools/docker-mcp/server.py`, значит
  свой репозиторий он может вычислить из `__file__`, и настройка для этого
  вообще не нужна. `EL_SCA_DIR` остаётся как переопределение — им гоняют сервер
  против другой рабочей копии.

  Заодно закрыт второй способ выстрелить себе в ногу: `EL_SCA_DIR=` (задана, но
  пустая) возвращала из `os.environ.get(name, default)` пустую строку, та
  становилась `Path(".")` и прибивала cwd к тому каталогу, где процесс
  случайно запустился. Теперь стоит `or`, и пустое значение уступает
  вычисленному корню.

  Тесты: три штуки в `tests/test_docker_mcp_server.py` — корень без переменной,
  переопределение переменной работает по-прежнему, пустое значение не побеждает.

- **Отчёт брал метаданные баз из чужого прогона** (CYBERSEC-14231).
  `_find_json_by_name()` искала артефакт через `rglob` и брала
  `sorted(...)[-1]` — последний путь по алфавиту. Канонический
  `artifacts/db_snapshot.json` пишет `run_summary` в корне каталога артефактов,
  и `manifest.py` читает его именно оттуда, но `artifacts/runs/…` сортируется
  ПОЗЖЕ, чем `artifacts/db_snapshot.json`, потому что `r` > `d`. Любая
  оставшаяся копия побеждала свежую.

  На прогоне jenkins.war 10.09.2026 победила копия от 14 июня из
  `artifacts/runs/PRE-CLEAN-20260706-20260706-180448/`. Отчёт написал
  `grype: built=2026-06-13T07:33:12Z` и `trivy: state=unknown` — при том, что
  прогон работал на grype `built 2026-09-09` и базе Trivy от 10.09
  (`UpdatedAt 2026-09-10T01:14:50Z`). Рядом стоял СВЕЖИЙ идентификатор снимка,
  `47bee95cbe1c`: его отчёт берёт из `summary.json`, другим путём. Именно это
  делало устаревшую аттестацию правдоподобной, и из-за неё прогон почти
  забраковали целиком.

  Теперь `root/<имя>` проверяется до рекурсивного поиска, а сам поиск
  канонический путь исключает. Порядок имён остался порядком предпочтения:
  каждое имя разрешается «корень, затем рекурсия» до перехода к следующему.
  Исправление касается всех четырёх канонических артефактов —
  `status.json`, `summary.json`, `run_manifest.json`, `db_snapshot.json`.

  Находок в отчёте это не меняло: числа брались из отчётов сканеров. Меняло
  аттестацию, а отчёт для заказчика и есть доказательство того, какими базами
  получены числа.

- **`.war` и `.jar` не распаковывались автоматически, и Java-поставка падала на
  первой же стадии** (CYBERSEC-14231). Список авто-распаковки в
  `scripts/run-scan.sh` знал только `tar/tgz/zip/rpm/deb`, поэтому для
  `jenkins.war` конвейер оставлял `EXTRACT=0`, Docker монтировал **файл** в
  `/scan-target`, а Syft запускался с `SYFT_FROM=dir` и умирал:

  ```
  ERROR could not determine source: errors occurred attempting to resolve '/scan-target':
    - local-directory: not a directory source: /scan-target
  [stage] sbom ошибка rc=1
  ```

  Про распаковку в сообщении нет ни слова, а `--resume` после такого падения
  промахивается второй раз: он пропускает стадию extract и оставляет цель на
  исходном файле. Отчёт от 27.08 по тому же `jenkins.war` получился только
  потому, что распаковку тогда включили руками флагом `-e`.

  Список расширений расширен (`.war`, `.jar`, `.ear`, `.hpi`, `.jpi`, `.aar`,
  `.whl`, `.egg`, `.nupkg`), но настоящее правило теперь другое: если имя ни о
  чём не говорит, скрипт спрашивает **содержимое** — тем же способом, которым
  решает сам экстрактор (`zipfile.is_zipfile` / `tarfile.is_tarfile` в
  `resilient_updates.extractor._archive_kind`). Поставка без расширения или с
  неожиданным расширением распакуется, обычный бинарник по-прежнему уйдёт в
  сканеры как есть.

  Тесты `tests/test_run_scan_autoextract.py`: текстовый контракт; проверка
  самого сниффера, **вынутого из скрипта** регуляркой, чтобы тест не разошёлся
  с кодом (zip под именем `.war`, zip под `.bin`, tar без расширения, обычный
  файл, несуществующий путь); и два поведенческих прогона настоящего
  `run-scan.sh` с подменённым `docker` — `.war` включает распаковку без `-e`,
  обычный файл не включает. Поведенческие помечены `slow` (каждый ~25 с).

- **Кириллица в тестах больше не красит master.** `ruff` дважды за три дня
  ронял `pre-commit` и `lint-python` на новом тестовом файле с русским
  docstring (RUF001/RUF002/RUF003), а из-за `needs`-гейтов вместе с ними не
  запускались `pytest` и `smoke` — то есть красный линт скрывал состояние
  тестов. Точечные исключения на каждый файл заменены одним правилом для
  `tests/**`; в рабочем коде правила остались. Попутно убраны три `noqa`,
  ставшие ненужными.
...
- **Фильтр коллизий добит: две подписи вместо одной** (CYBERSEC-14277).
  ...
  причинам, и обе теперь закрыты.

  *Компонент никогда не публиковался.* У собственного `-SNAPSHOT`-модуля нет
  Maven-координат, поэтому Syft синтезирует groupId из имени файла и purl
  выглядит как `pkg:maven/core/core@3.29.3-SNAPSHOT`. Артефакта с таким именем
  нет ни в одном реестре — значит им не владеет ни один вендор NVD, и любая
  привязка по голому имени является коллизией, включая `vendor=unknown`.
  Легаси-groupId вроде `log4j:log4j` под правило не попадают: это публикации,
  а не SNAPSHOT-сборки.

  *`unknown` в опровергнутом веере.* `vendor=unknown` — штатный вывод
  cve-bin-tool и сам по себе не повод отбрасывать находку. Но если именованные
  вендоры на той же паре (продукт, версия) только что опровергнуты groupId, то
  безымянные — часть того же веера по голому имени и уходят вместе с ними.
  Компонент без опровергнутых соседей (`unknown/netty-codec-http`) не трогаем.

  Каждая отсеянная находка несёт `dropped_evidence`, и колонка «groupId
  evidence» в отчёте берёт его вместо разбора текста причины.

  Итог по агенту 3.29.3 на исправленном конвейере (Trivy уже в режиме
  `rootfs`): Grype 27, Trivy 134, cve-bin-tool 38 при 496 отсеянных —
  **199 находок, 0 CRITICAL, вердикт `pass`**.
  Тесты: `tests/test_collision_filter_unpublished.py` (10) — обе подписи плюс
  контроль, что настоящие находки и легаси-groupId не задеты.
