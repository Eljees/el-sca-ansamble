...
- **Отчёт перестал считать чужие библиотеки своими** (CYBERSEC-14277). Вендор
  Агента СМЭВ4 заявил, что в нашем отчёте есть `bcprov-jdk18on` и
  `woodstox-core`, которых он не поставляет. Проверка подтвердила его правоту
  и вскрыла два независимых дефекта, оба раздували Java-отчёты годами.

  *Фантомы из встроенных pom.xml.* Внутри чужих jar лежат
  `META-INF/maven/**/pom.xml` — паспорт самой библиотеки со списком её
  **объявленных** зависимостей. Каталогизатор Syft `java-pom-cataloger` и
  анализатор Trivy `type=pom` читали эти файлы и записывали перечисленное как
  присутствующие компоненты, хотя сборщик их в поставку не включил. На
  агенте 3.29.3 таких «бумажных» компонентов оказалось 627 из 1542; на них
  пришлось 69 находок Grype из 96 и **все 19** находок Trivy. Проверено
  физически: прошли 507 вложенных архивов, искали и `bcprov*.jar`, и классы
  `org/bouncycastle/**`, `com/ctc/wstx/**` — ноль совпадений. Характерный
  признак фантома — версии, которых в современной сборке быть не может:
  `logback-core 0.9.24`, `log4j 1.2.13`, `commons-io 1.4`, `hsqldb 2.4.0`.
  Лечение: `--select-catalogers ${SYFT_SELECT_CATALOGERS:--java-pom-cataloger}`
  у `syft-sbom` и `--skip-files **/META-INF/maven/**/pom.xml` в обеих ветках
  `update_trivy.sh` (выключается через `TRIVY_SKIP_EMBEDDED_POM=0`).
  Результат: Grype 96 → 27, Trivy 19 → 0, **ни одной новой находки**.

  *Коллизия по родовому имени продукта.* Модули поставки называются
  `notarius-ng-core`, `einfahrt-core` и т.п.; после разбора имени продуктом
  становится слово `core`, а в NVD под ним зарегистрированы Drupal core,
  MobileIron Core, ONLYOFFICE core. Их уязвимости и приписывались модулям
  заказчика: CVE-2016-9450 — отравление кэша в форме сброса пароля Drupal 8.x —
  стояла на компоненте `core 3.29.3-SNAPSHOT`. Так набралось 492 находки из
  534, включая **все 62 CRITICAL**, то есть каждая критическая уязвимость в
  отчёте была ложной. `reporting.py` получил `_GENERIC_PRODUCT_NAMES` и
  `_is_name_collision()`; правило намеренно простое — родовое имя недостоверно
  независимо от вендора, потому что находки приходили и с `vendor=unknown`, и
  с `mobileiron`/`onlyoffice`/`ractf`. Отсеянное **не удаляется**, а выводится
  в раздел «Отсеяно как совпадение по имени продукта» с вендором, продуктом и
  количеством; `cve_count` уменьшается на число отсеянных, иначе шапка
  расходится с таблицей. Риск потерять настоящую находку прикрыт: те же
  компоненты независимо проверяются Grype и Trivy по purl/CPE.

  Итог по агенту 3.29.3: **745 находок / 77 CRITICAL → 69 / 0**, вердикт
  политики `fail: CRITICAL=77>0` → `pass`. Прежнее заключение «ПО не может
  быть использовано в контуре» держалось на ложных срабатываниях.
